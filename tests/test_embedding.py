...
from decisions.embedding import (
    EmbeddingError,
...
class FakeHTTPClient:
    ...
    def post(self, url, headers, json):
        ...
        return httpx.Response(
            200,
            json={"data": [{"embedding": embedding} for embedding in self.embeddings]},
...
def test_voyage_embedding_client_batches_inputs():
    http_client = FakeHTTPClient()
    http_client.embeddings = [[0.1], [0.2]]
    client = VoyageEmbeddingClient(
        api_key="voyage-key",
        model="voyage-3.5-lite",
        output_dimension=1024,
    ...
    assert client.embed_many(["First chunk.", "Second chunk."]) == [[0.1], [0.2]]
...
def test_extract_embeddings_rejects_count_mismatch():
    payload = {"data": [{"embedding": [1]}, {"embedding": [2]}]}

    with pytest.raises(EmbeddingError, match="count"):
        _extract_embeddings(payload, expected_count=1)
